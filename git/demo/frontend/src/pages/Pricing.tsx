...
export default function Pricing() {
  // On bot.goalkeeper91.de (the Paddle-facing storefront, see
  // lib/botDomain.ts) this page is also the redirect target for "/" - it
  // must show ONLY the Twitch Bot tiers, never the freelance-development
  // section below, since that section is exactly the product category
  // Paddle rejected the main domain for.
  ...
  return (
    <div className="relative w-full min-h-screen bg-slate-950 text-white">
      {isBot ? (
        <Seo
          title="Preise - Twitch Bot SaaS"
          description="Twitch-Bot-Abo von Goalkeeper91 - Automod, Loyalty-Punkte, Giveaways, Chat-Commands und automatische Clip-Erstellung. Monatlich oder jährlich, jederzeit kündbar."
          path="/pricing"
        />
      ) : (
        <Seo
      ...
      {/* Intro + Sprungnavigation - orientiert sofort, dass hier zwei getrennte Angebote stehen.
          Auf bot.goalkeeper91.de (siehe lib/botDomain.ts) gibt es nur das eine Angebot, also
          auch nur die schlichtere Intro ohne Verweis auf die Softwareentwicklung. */}
      {isBot ? (
        <section className="text-center pt-16 pb-10 px-6">
          <h1 className="text-4xl sm:text-5xl font-extrabold mb-4">Twitch Bot - Preise</h1>
          <p className="text-gray-300 max-w-2xl mx-auto mb-8">
            Ein SaaS-Abo für deinen Twitch-Kanal mit festen monatlichen/jährlichen Tarifen - jederzeit kündbar.
          </p>
        </section>
      ) : (
        <section className="text-center pt-16 pb-10 px-6">
      ...
      {/* Bewusst visuell abgesetzt (eigener Hintergrund) vom SaaS-Abo oben,
          ...
          Komplett ausgeblendet auf bot.goalkeeper91.de (siehe lib/botDomain.ts) -
          dieser Abschnitt beschreibt exakt die Produktkategorie
          ("Software Development Services"), wegen der Paddle die Hauptdomain
          abgelehnt hat. */}
      {!isBot && (
